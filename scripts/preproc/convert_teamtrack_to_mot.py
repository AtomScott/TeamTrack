...
def process_csv_file(csv_file, output_dir, dataset_name, subset, subset_dir):
    ...
    while True:
        ...
        )  # create a name of each image: 000001.jpg, 000002.jpg and so on
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert TeamTrack data to MOTChallenge format."
    )
    parser.add_argument(
        "--teamtrack-dir",
        type=str,
        default="./datasets/teamtrack",
        help="Path to the TeamTrack dataset.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="./datasets/teamtrack-mot",
...
def main(args):
    ...
    for dataset_name in dataset_names:
        ...
        for subset in ["train", "test", "val"]:
            ...
            assert os.path.exists(
                subset_dir
            ), f"Subset directory {subset_dir} does not exist."

            csv_files = glob(os.path.join(subset_dir, "annotations", "*.csv"))
            dirs[subset] = Parallel(n_jobs=args.n_jobs)(
                delayed(process_csv_file)(
                    csv_file, output_dir, dataset_name, subset, subset_dir
                )
