...
def main(cfg):
    ...
    def objective(trial):
        start_time = time()
        parameters = {}
        parameters["tracker_type"] = cfg.TRACKER.YOLOV8.TRACKER_TYPE
        ...
        parameters['conf'] = trial.suggest_float('conf', 0.1, parameters["track_low_thresh"])
        ...
        for video_path in video_paths:
            ...
            # check if the detections are null
            results = np.loadtxt(output_path, delimiter=',')
            if len(results) == 0:
                logger.warning(f"No detections found, skipping evaluation for {output_path}")
    ...
    study = optuna.create_study(directions=["maximize", "minimize"], sampler=sampler, study_name=study_name, storage=storage_name)
